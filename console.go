...
func (w ConsoleWriter) Write(p []byte) (n int, err error) {
	...
	//添加字段
	w.writeFields(evt, buf, p)

	if MessageFlag == false { //msg放到最后
		buf.WriteByte(' ')
		w.writePart(buf, evt, zerolog.MessageFieldName)
	}
...
// p原始数据
func (w ConsoleWriter) writeFields(evt map[string]interface{}, buf *bytes.Buffer, p []byte) {
